    ...
    <div className="space-y-5">
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`rounded-xl py-2 text-sm font-semibold transition ${
              tab === t.key ? 'bg-white text-slate-900 shadow' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            {t.label}
          </button>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {view.transfers.length === 0 ? (
          <p className="rounded-xl bg-emerald-50 px-4 py-6 text-center font-medium text-emerald-700">
        ...
        <button
          onClick={copySummary}
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-300 bg-white py-3 font-semibold text-slate-700 transition hover:border-slate-400"
        >
        ...
        <button
          ...
          className="mt-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 py-3 font-bold text-white transition active:scale-[0.98]"
        >
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <ul className="space-y-2">
          {people.map(([name, p]) => {
            ...
            return (
              <li key={name} className="rounded-2xl bg-slate-50 p-3">
